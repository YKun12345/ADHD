...
复审加固实际补充：认证请求使用页面活动状态、操作代次、患者数据修订号和 API 来源租约阻止旧响应写回；新凭证与 App 内存态统一提交，失败时回滚；切换服务器前必须完整结束旧来源会话；当前会话 401、退出和服务器切换均区分凭证残留、页面清空失败及导航失败；安全跳转统一处理同步异常和异步失败。最终在功能分支和合并后的 `main` 均通过 53 个测试文件、93 个 JavaScript 文件语法检查、全部 JSON 解析、16 条路由和 92 个 WXML 事件绑定检查。

2026-08-24 微信开发者工具编译修复：开发者工具 3.16.2 报告 `module 'utils/safe-navigation.js' is not defined`，导致首页页面脚本未注册并只显示空白背景。按 TDD 先把安全导航测试改为从稳定直载的会话模块导入并观察失败，再把跳转逻辑并入 `session-privacy.js`，更新登录、注册、服务器设置和请求层引用，删除易被增量编译漏打包的独立模块。提交 `1f748f4` 后，合并后的 `main` 再次通过 53 个测试文件、93 个 JavaScript 文件和 22 个 JSON 文件检查；微信开发者工具重新编译后的可视恢复仍由 A 人工确认。
