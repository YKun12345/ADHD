...
- **Task 1：已完成。** `asrs-scale.test.js` 首次因模块不存在出现 `MODULE_NOT_FOUND`，随后迁移成人 18 题并完成答案规则。提交：`11ece1d`。
- **Task 2：已完成。** `scale-page.test.js` 首次因页面不存在出现 `MODULE_NOT_FOUND`，随后实现成人初始化、儿童保护、草稿、导航、结果结构校验、失败保留和重复提交保护。提交：`c28e1c1`。
- **Task 3：已完成。** `scale-view.test.js` 首次因 WXML 不存在出现 `ENOENT`，随后完成单题卡、五级选项、进度、结果和免责声明。提交：`709348e`。
- **Task 4：已完成。** 成人入口测试先得到 `false !== true` 红灯，随后在首页启用 `/pages/scale/index`，并保持儿童入口为“D5 开放”。提交：`2e05d13`。
- **Task 5：已完成。** 11 个小程序测试文件和 25 个 JavaScript 文件语法检查全部通过，`git diff --check` 通过。真实服务端算分没有用 mock 冒充完成，仍记录为 B 接口联调边界；D4 微信开发者工具视觉复核统一留到 D14 UI 验收。
- **范围确认：** 本阶段未修改 `backend/`、数据库、医生 Web 或部署文件。
