...
- [ ] 先测试 10 轮（6 GO、4 NO-GO）、五种判定、汇总和 payload，观察模块不存在红灯。
- [ ] 实现 `TRIAL_SEQUENCE/evaluateTrial/summarizeTrials/buildCognitivePayload`。
- [ ] 运行测试、语法检查并提交 `feat(miniprogram): add Go No-Go test model`。
...
- [ ] 先用 fake `setTimeout/clearTimeout/Date.now` 测试启动、提前点击、GO 点击、NO-GO 超时、完成、同步失败待保存、重复启动和卸载清理。
- [ ] 实现 `intro/waiting/stimulus/feedback/result` 阶段机和两个定时器句柄。
- [ ] 运行页面与纯逻辑测试，语法检查并提交 `feat(miniprogram): implement Go No-Go controller`。
...
- [ ] 先测试说明、进度、GO/NO-GO 刺激、触摸区域、反馈、结果指标、同步状态和免责声明，观察文件不存在红灯。
- [ ] 创建适合手机触摸的高对比测试区，禁用文本选择和重复按钮。
- [ ] 运行视图与控制器测试并提交 `style(miniprogram): build Go No-Go test interface`。
...
- [ ] 先要求认知任务和快捷入口可用并指向 `/pages/cognitive/index`，观察红灯。
- [ ] 注册页面并为成人、儿童启用认知入口；未知患者类型保持锁定。
- [ ] 回归后提交 `feat(miniprogram): enable Go No-Go entry`。
...
- [ ] 运行全部小程序测试、全部 JavaScript 语法和 `git diff --check`。
- [ ] D6 更新为 100%，记录提交、同步边界、测试数量和真机触摸待 D13 验收。
- [ ] 提交记录并确认工作区干净。
